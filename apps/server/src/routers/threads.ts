...
export async function threadRoutes(fastify: FastifyInstance) {
	...
	fastify.get("/threads/:id", async (request, reply) => {
		...
		try {
			...
			if (!thread) {
				return reply
					.status(404)
					.send({ success: false, error: "Thread not found or deleted" });
			}
	...
	fastify.get("/threads/topic/:topicId", async (request, reply) => {
		...
		if (!query.success)
			return reply
				.status(400)
				.send({ success: false, error: "Invalid pagination parameters" });
		...
		try {
			...
			const [countResult] = await DrizzleClient.select({
				count: sql<number>`cast(count(${threadsTable.id}) as integer)`.as(
					"count",
				),
			})
				.from(threadsTable)
	...
	fastify.delete(
		...
		async (request, reply) => {
			...
			if (!deletedThread) {
				return reply
					.status(500)
					.send({ success: false, error: "Failed to perform soft delete" });
			}

			return reply.status(200).send({
				success: true,
				message: `Thread ${deletedThread.id} soft-deleted.`,
